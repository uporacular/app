...
var ConsentService = (function () {
  ...
  function _hash(subjectId) {
    var value = String(subjectId || '').trim();
    if (!value) throw new Error('[ConsentService] Sujeito obrigatorio.');
    var bytes = Utilities.computeDigest(
      Utilities.DigestAlgorithm.SHA_256,
      value,
      Utilities.Charset.UTF_8
    );
    return 'cs-' + Utilities.base64EncodeWebSafe(bytes).replace(/=+$/, '');
  }
  ...
  function check(subjectId, purpose) {
    if (VALID_PURPOSES.indexOf(purpose) === -1) {
      throw new Error('[ConsentService] Finalidade invalida: "' + purpose + '".');
    }
  ...
  function grant(subjectId, purpose, opts) {
    ...
    if (!responsible || responsible.length > 120) {
      throw new Error('[ConsentService] Responsavel obrigatorio (ate 120 caracteres).');
    }
  ...
  function revoke(subjectId, purpose, opts) {
    try {
      ...
      var updated = false;
      for (var i = data.length - 1; i >= 1; i--) {
        if (String(data[i][1]) === sh && String(data[i][2]) === purpose && String(data[i][6]) === 'active') {
          sheet.getRange(i + 1, 7).setValue('revoked');
          sheet.getRange(i + 1, 8).setValue(now.toISOString());
          sheet.getRange(i + 1, 9).setValue(String(opts.reason || '').slice(0, 300));
          updated = true;
          break;
        }
      }
      if (!updated) throw new Error('[ConsentService] Consentimento ativo nao encontrado.');
   ...
   * @param {string} subjectId
   * @param {string} purpose
   * @param {{fresh?: boolean}} options Ignora cache para operações anuais sensíveis.
   * @returns {{ status: string, subjectHash: string, record: Object|null }}
   */
  function getStatus(subjectId, purpose, options) {
    var sh     = _hash(subjectId);
    var cached = options && options.fresh ? null : _getCache(sh, purpose);
